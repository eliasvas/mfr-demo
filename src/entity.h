...
//this are so that we can move the exported points!!! --look at game.c to learn more
extern vec3 middle;
extern vec3 points_position;
extern u32 points_count;
b32 points_collided_prev_frame;
...
entity_manager_update(EntityManager *manager, Renderer *rend)
{
    ...
    for (u32 i = 0; i < manager->model_manager.next_index; ++i)
    {
        ...
        if ((collision||points_collided_prev_frame) && global_platform.right_mouse_down)
        {
            vec3 right = vec3_normalize(vec3_cross(rend->cam.front, rend->cam.up));
            vec3 up = vec3_normalize(rend->cam.up);
            ...
            points_collided_prev_frame = TRUE;
            //sprintf(error_log, "collision detected!!");
        }else
            points_collided_prev_frame = FALSE;
}
internal void 
entity_manager_render(EntityManager *manager, Renderer *rend)
{
    if (rend->point_alloc_pos == 0)
